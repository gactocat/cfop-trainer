...
export function OLLRandomTrainer() {
  const { t, tn } = useT();
  const { add, all, bestFor, ao5For, solvesFor } = useOLLRandomSolves();
  const { starredFor } = useOLLAlgorithms();
  ...
  // Epoch ms of each OLL's most recent solve, driving the staleness-weighted
  ...
  const lastRecorded = useMemo(() => {
    ...
    for (const s of all) {
      ...
      const prev = map.get(s.ollId);
      if (prev === undefined || ts > prev) map.set(s.ollId, ts);
    }
    return map;
  }, [all]);

  // AUF for the picked OLL comes from its starred algorithm in All OLLs mode,
  // so the random case is presented in the orientation the user actually
  // practices. Falls back to U0 if no algorithm is starred for that OLL.
  ...
  const pickRandom = useCallback((): Pick => {
    const pool = OLL_IDS.filter((id) => selected.has(id));
    const list = pool.length > 0 ? pool : OLL_IDS;
    const ollId = pickStaleWeighted(list, (id) => lastRecorded.get(id), Date.now());
    const auf = starredFor(ollId)?.auf ?? 'U0';
    return { ollId, auf };
  ...
  const record = useCallback(() => {
    if (current && elapsed > 0) add(current.ollId, elapsed);
    setState('idle');
    setElapsed(0);
    setCurrent(null);
  }, [current, elapsed, add]);
  ...
  if (state === 'stopped' && current) {
    const def = getOLLDefinition(current.ollId);
    const star = starredFor(current.ollId);
    // Random-trainer history for this case so far (the time on screen is not
    // recorded yet).
    const previousSolves = solvesFor(current.ollId).length;
    return (
      <div className="w-full flex-1 rounded-lg border border-amber-400 dark:border-amber-500 bg-amber-50 dark:bg-amber-950/30 p-6 flex flex-col items-center justify-center gap-6 select-none">
        <div className="flex flex-col sm:flex-row items-center gap-6">
          <div className="rounded-md p-2 bg-zinc-100 dark:bg-zinc-900">
            <OLLLLView ollId={current.ollId} auf={current.auf} size={220} />
          </div>
          <div className="text-center sm:text-left">
            <div className="text-sm uppercase tracking-wider text-amber-700 dark:text-amber-300">
              {def?.name ?? current.ollId} · {current.auf}
            </div>
            ...
            {previousSolves > 0 && (
              <div className="mt-1 text-xs font-mono text-amber-700/80 dark:text-amber-300/80">
                {t('detail.randomStats', {
                  best: formatSeconds(bestFor(current.ollId)),
                  ao5: formatSeconds(ao5For(current.ollId)),
        ...
        <div className="max-w-2xl w-full text-center font-mono text-sm break-words text-zinc-700 dark:text-zinc-300">
          {star ? (
            ...
            <p className="italic text-zinc-500">{t('trainer.oll.noStar')}</p>
          )}
        </div>
  ...
  if (noneSelected) {
    return (
      <div className="w-full flex-1 min-h-[200px] rounded-lg flex flex-col items-center justify-center gap-2 select-none bg-zinc-200 dark:bg-zinc-800 text-zinc-500">
        ...
        <div className="text-xs">{t('trainer.oll.tickOne')}</div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="text-xs opacity-75">
        {t('trainer.oll.hidden')}
        {mounted &&
          ` · ${t('trainer.selectedCount', { count: selected.size, total: OLL_IDS.length })}`}
      </div>
    </button>
  );
}
